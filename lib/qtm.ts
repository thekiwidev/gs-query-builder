 ...
 */
export function buildScholarUrl(blocks: SearchBlock[], globalFilters?: GlobalFilters): QTMResult {
  ...
  const validBlocks = blocks.filter(block => 
    block.term && block.term.trim().length > 0 && block.fieldId
  );
  ...
  try {
    ...
    for (const block of validBlocks) {
      ...
      if (synthesizedBlock) {
        ...
        result.messages.push(`Failed to process block with field ID: ${block.fieldId}`);
      }
    }
    ...
    if (finalUrl.length > MAX_URL_LENGTH) {
      result.messages.push(`Warning: Generated URL length (${finalUrl.length}) exceeds recommended limit (${MAX_URL_LENGTH})`);
    }
    ...
    if (result.messages.length === 0) {
      result.messages.push(`Successfully generated query with ${synthesizedBlocks.length} search blocks`);
    }

  } catch (error) {
    result.messages.push(`Error building URL: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
